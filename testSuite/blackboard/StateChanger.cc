...
void StateChanger::initialize(int stage)
{
    ...
    if(stage == 0) {
        ...
        catHostState = bb->getCategory(&hs);
        catTestParam = bb->getCategory(&tp);
        hs.setState(HostState::SLEEP);
        tp.setState(TestParam::BLUE);
        bb->publishBBItem(catHostState, &hs, getParentModule()->getId());
        bb->publishBBItem(catTestParam, &tp, getParentModule()->getId());
    }
}
