 ...
 */
void BasicMobility::handleSelfMsg(cMessage * msg)
{
    makeMove();
    updatePosition();
 ...
 * playground.
 **/
void BasicMobility::handleBorderMsg(cMessage * msg)
{
...
{
    Coord p;
    p.x = uniform(0, cc->getPgs()->x, 0);
    p.y = uniform(0, cc->getPgs()->y, 0);
    return p;
}
 ...
 **/
void BasicMobility::reflectIfOutside(BorderHandling wo, Coord& stepTarget, Coord& targetPos, Coord& step, double& angle)
{
    switch( wo ){
    case X_SMALLER:
 ...
 * @param targetPos target position of the host (for non atomic movements)
 **/
void BasicMobility::wrapIfOutside(BorderHandling wo, Coord& stepTarget, Coord& targetPos)
{
    switch( wo ){
    case X_SMALLER:
 ...
 * started.
 **/
BasicMobility::BorderHandling BasicMobility::checkIfOutside( Coord targetPos, Coord& borderStep )
{
 ...
 * calculated.
 **/
void BasicMobility::goToBorder(BorderPolicy policy, BorderHandling wo, Coord& borderStep, Coord& borderStart)
{
    double factor;
 ...
 **/
bool BasicMobility::handleIfOutside(BorderPolicy policy, Coord& stepTarget, Coord& targetPos, Coord& step, double& angle)
{
