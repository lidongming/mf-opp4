...
void NicEntryDebug::connectTo(NicEntry* other)
{
    ...
    outConns[other->nicId] = localoutgate->getPathStartGate();
}
...
cGate* NicEntryDebug::requestInGate(void)
{
  ...
  if( !freeInGates.empty() ){
    ...
    // if there is no snrEval module we should have a phy getModule(P2PPhyLayer)
    if( (phyModule = static_cast<ChannelAccess *>(nicPtr->getSubmodule("snrEval"))) == NULL )
      phyModule = static_cast<ChannelAccess *>(nicPtr->getSubmodule("phy"));
    assert(phyModule != 0);
