...
void ChannelAccess::initialize( int stage )
{
    ...
    if( stage == 0 ){
        ...
        catHostMove = bb->subscribe(this, &hostMove, findHost()->getId());
        isRegistered = false;
    }
}
...
void ChannelAccess::sendToChannel(cMessage *msg, simtime_t delay)
{
    ...
    gateList = cc->getGateList( getParentModule()->getId(), &hostMove.startPos );
...
/*
        ...
        if( i != gateList.end() ){
            for(; i != --gateList.end(); ++i){
                ...
                for (int g = radioStart; g != radioEnd; ++g)
                    sendDirect(static_cast<cMessage*>(msg->dup()),
                               delay, i->second->getOwnerModule(), g);
            ...
            sendDirect(msg, delay, i->second->getOwnerModule(), radioEnd);
        }
        else{
...
{    
    ...
    if(category == catHostMove)
    {
        ...
        if(isRegistered) {
            cc->updateNicPos(getParentModule()->getId(), &hostMove.startPos, &hm.startPos);
        }
        else {
            // register the nic with ChannelControl
            // returns true, if sendDirect is used
            useSendDirect = cc->registerNic(getParentModule(), &hm.startPos);
            isRegistered = true;
        }
        hostMove = hm;
