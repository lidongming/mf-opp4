...
void ChannelControl::initialize()
{
  ...
  if(useTorus) {
      coreEV<<"wrapping connections around playground edges"<<endl;
  }
  else {
      coreEV<<"not wrapping connections around playground edges"<<endl;
  }
  ...
  if((numX <= 3) && (numY <= 3))
  {
      ...
      coreEV <<" using "<<numX<<"x"<<numY<<" grid"<<endl;
  }
  maxX = nics.size()-1;
  maxY = nics[0].size()-1;
}
...
bool ChannelControl::registerNic( cModule* ptr, const Coord* pos )
{
  ...
  NicEntry *nic;
  int id = ptr->getId();
  coreEV <<" registering nic #"<<id<<endl;
  ...
  nic->nicPtr = ptr;
  nic->nicId = id;
  nic->hostId = ptr->getParentModule()->getId();
...
void ChannelControl::updateNicPos(int id, const Coord* oldPos, const Coord* newPos)
{
    ...
    coreEV <<"nic #"<<id<<" moved from " << oldPos->info() << " to " << newPos->info() << " pgs: " << playgroundSize.info() << "\n";
...
//    coreEV <<"nic #"<<id<<" moved from " << oldPos->info() << " to " << newPos->info() << " pgs: " << playgroundSize.info() << " oldX: "<< oldX << " oldY: " << oldY << " newX: " << newX << " newY: " << newY << "\n";
    // update all connections for this nic
    ...
    
{
    ...
    for(gUmIt = gridUnion.begin(); gUmIt != gridUnion.end(); ++gUmIt)
    {
        ...
        {
            cY = *gUsIt;
            coreEV << "Update cons in ["<<cX<<", "<<cY<<"]"<<endl;
...
void ChannelControl::updateConnections(NicEntries& nmap, NicEntry* nic)
{
    ...
    for(NicEntries::iterator i=nmap.begin(); i!=nmap.end(); ++i)
    {
        ...
        if ( inRange && !nic->isConnected(nic_i) ){
            ...
            coreEV <<"nic #"<<nic->nicId<<" and #"<<nic_i->nicId << " are in range"<<endl;
            nic->connectTo( nic_i );
            nic_i->connectTo( nic );
        }
            ...
            coreEV <<"nic #"<<nic->nicId<<" and #"<<nic_i->nicId << " are NOT in range"<<endl;
...
void ChannelControl::finish()
{
  NicMatrix::iterator it;
  RowVector::iterator rowIt;
  NicEntries::iterator mapIt;
  for(it = nics.begin(); it != nics.end(); it++) {
    for(rowIt = it->begin(); rowIt != it->end(); rowIt++) {
      for(mapIt = rowIt->begin(); mapIt != rowIt->end(); mapIt++)
        delete (mapIt->second);
    }
  }
}
