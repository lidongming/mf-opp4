...
void MassMobility::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        step.x = move.speed * cos(PI * currentAngle / 180) * updateInterval.dbl();
        step.y = move.speed * sin(PI * currentAngle / 180) * updateInterval.dbl();
...
void MassMobility::handleSelfMsg(cMessage * msg)
{
    ...
    switch (msg->getKind()){
    case MOVE_HOST:
	BasicMobility::handleSelfMsg( msg );
	break;
    case MK_CHANGE_DIR:
