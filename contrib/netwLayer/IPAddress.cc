...
// Copyright (C) 2007 Jerome Rousselot
//
...
//  Import into Mobility Framework v2: Jerome Rousselot, CSEM, 2007
//
...
void IPAddress::set(unsigned int ip)
{
    addr[0] = (ip >> 24) & 0xFF;
    addr[1] = (ip >> 16) & 0xFF;
    addr[2] = (ip >> 8) & 0xFF;
...
bool IPAddress::prefixMatches(const IPAddress& to_cmp, int numbits) const
{
    ...
    unsigned int addr1 = getInt();
    unsigned int addr2 = to_cmp.getInt();
