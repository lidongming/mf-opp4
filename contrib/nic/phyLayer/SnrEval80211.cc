...
double SnrEval80211::calcDuration(cMessage *frame)
{
    ...
    EV << "bits without header: " << f->getBitLength() - headerLength
       << ", bits header: " << headerLength << endl;
    duration = ((f->getBitLength() - headerLength) / f->getBitrate() + headerLength / BITRATE_HEADER);
    return duration;
}
...
{
    AirFrame80211 *frame = new AirFrame80211(msg->getName(), msg->getKind());
