...
{
    double duration;
    assert(static_cast<cPacket*>(af));    
    cPacket * pkt = static_cast<cPacket*>(af);
    duration = (double)pkt->getBitLength()/(double)bitrate;
    return duration;
}
 ...
 **/
void P2PPhyLayer::sendDelayedP2P(AirFrame* frame, simtime_t delay, int dest)
{
...
/*
	if(useSendDirect){
	    ...
	    //use send
	    sendDelayed( (cMessage*)frame , delay, outGate->getId());
	}
*/
	    //always use send with omnet++4.0
	    sendDelayed( (cMessage*)frame , delay, outGate->getId());
 ...
 *
 * @param dest Destination nic module getId() in order to find the
 * destination gate for sending the message
 **/
void P2PPhyLayer::sendDelayedDown(AirFrame* msg, simtime_t delay, int dest)
{
  if(dest == -1){
    //send broadcast msg
 ...
 **/
void P2PPhyLayer::sendDelayedUp(AirFrame* msg, simtime_t delay)
{
...
{
    double pktError=pow(1-pBit,(int)msg->getBitLength());
    bool ok=bernoulli(pktError, 0);

    //send the packet up if it was received correctly - otherwise delete it
    if(ok){
        EV <<"msg "<<msg->getName()<<" forwarded to upper layer\n";
        sendUp(msg);
    }
    else{
        EV <<"Sorry, msg "<<msg->getName()<<" lost due to biterrors\n";
        delete msg;
    }
}
