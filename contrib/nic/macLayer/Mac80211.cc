...
void Mac80211::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        catMedium = bb->subscribe(this, &ind, getParentModule()->getId());
...
Mac80211Pkt *Mac80211::encapsMsg(cMessage * netw)
{
    ...
    pkt->setBitLength(MAC80211_HEADER_LENGTH);                  // headerLength, including final CRC-field
    ...
    assert(static_cast<cPacket*>(netw));
    pkt->encapsulate(static_cast<cPacket*>(netw));
...
{
    EV << "Mac80211::handleLowerControl " << msg->getName() << "\n";
    switch(msg->getKind()) {
    case COLLISION:
    case BITERROR:
...
{
    EV << "Mac80211::handleSelfMsg " << msg->getName() << "\n";
    switch (msg->getKind())
    {
    case END_SIFS:
...
void Mac80211::sendRTSframe()
{
    ...
    frame->setDuration(3 * SIFS + packetDuration(LENGTH_CTS, br) +
                       packetDuration(fromUpperLayer.front()->getBitLength(), br) +
 ...
 */
simtime_t Mac80211::timeOut(frameType_802_11 type, double br)
{
...
void Mac80211::suspendContention()  {
    ...
    if(contention->isScheduled()) {
        ...
        if(simTime() - contention->getSendingTime() < DIFS) {
            remainingBackoff = (contention->getArrivalTime() - contention->getSendingTime() - DIFS);
        }
        else {
            remainingBackoff = ((int)((contention->getArrivalTime().dbl() - simTime().dbl() - DIFS)/ST)) * ST;
...
void Mac80211::addNeighbor(Mac80211Pkt *af) {
    ...
    double snr = static_cast<PhyControlInfo *>(af->getControlInfo())->getSnr();
